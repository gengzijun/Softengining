...
public class SavingGoalPage {
    ...
    public SavingGoalPage(Stage stage, HostServices host) throws CsvValidationException, IOException {
        ...
        unifiedConfirm.setOnAction(e -> {
            // Execute the first action (handle confirm for progress)
            progressBox.handleConfirm();

            // Use Platform.runLater to ensure the second action is executed after the first
            Platform.runLater(() -> {
                // Trigger the second action (save input value for emergency box)
                emergencyBox.saveInputValue();
            });
        });
    }
